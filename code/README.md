...
Pipeline that verifies damage claims (car / laptop / package) using submitted images, claim conversation, user history, and evidence requirements. Produces structured CSV predictions for HackerRank Orchestrate.
...
Never commit `code/.env`. API keys are read from environment variables only.
